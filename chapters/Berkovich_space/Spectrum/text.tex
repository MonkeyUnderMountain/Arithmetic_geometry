...
\subsection{Spectrum of Tate algebras}
    ...
    % Here is a picture illustrating the four types of points in the spectrum \(\scrM(\kkk\{T\})\):
    ...
    \begin{example}\label{eg:Tate_algebra_in_1_variable_under_reduction_map}
        ...
        Here we describe the reduction map and the kernel map for the spectrum \(\scrM(A)\).
    ...
    \begin{example}\label{eg:Tate_algebra_in_1_variable_under_kernel_map}
        Here we describe the kernel map for the spectrum \(\scrM(A)\).
        If \(x = x_a\) is a type I point corresponding to \(a \in \kkk\) with \(|a| \leq r\), then \(\wp_{x_a} = (T - a)\).
        For type II, type III or type IV points \(x\), note that if \(f \in A\) is invertible, then \(|f|_x > 0\) for each \(x \in \scrM(A)\) since \(|f|_x |f^{-1}|_x = |1| = 1\).
        ...
        \end{cases} \]
        % under the kernel map \(\scrM(A) \to \Spec(A)\), all type II, type III and type IV points are mapped to the zero ideal \((0)\) and each type I point \(x_a\) is mapped to the maximal ideal \((T - a)\).
        % \(A\) is an eculidean domain and hence a principal ideal domain.
        % We have 
        % \[ \Spec(A) = \{(0)\} \cup \{(T-c) : c \in \kkk, |c| \leq r\}. \]
        % and the topology on \(\Spec(A)\) is such that the closed subsets are the whole space \(\Spec(A)\), emptyset and finite subsets of maximal ideals. 
        % For a type I point \(x_a\) with \(a \in \kkk\) and \(|a| \leq r\), we have \(\wp_{x_a} = (T - a)\).
        % For a type II or type III point \(x_{a,s}\), since \(|f|_{x_{a,s}} = 0 \implies f(b) = 0\) for each \(b \in E(a,s)\) and hence \(f=0\), we have \(\wp_{x_{a,s}} = (0)\).
        % For a type IV point \(x_{\{a_n,s_n\}}\), note that if \(f \in A\) is invertible, 
    \end{example}

    \paragraph{Spectrum of Tate algebra in servel variables} 
    Let \(A = \kkk\{\underline{T}\}\).
    To consider the spectrum \(\scrM(A)\), inspired by \cref{eg:Tate_algebra_in_1_variable_under_kernel_map}, 

...
\subsection{Definition}
    ...
    \begin{example}\label{eg:spectrum_of_Z_with_absolute_value_norm}
        ...
        \begin{tikzpicture}[scale=1, every node/.style={font=\small}]
            ...
            % archimedean branch
            \node[circle, fill=black, inner sep=1pt, label=above:{\(|\cdot|_\infty\)}] (archimedean_top) at (0,1) {};
            ...
            \foreach \p/\q in {2/0.5,3/0.667,5/0.8,7/0.857,11/0.909} {

                \node[circle, fill=black, inner sep=1pt, label=below:{\(|\cdot|_{\p}^\infty\)}] (p_top_\p) at (-4 + \p,-2) {};
                \draw (root) -- (p_top_\p);
                \node[circle, fill=magenta, inner sep=1pt, label=left:{}] at (-4*\q + \p*\q,-2*\q) {};
            }
            \node[circle, fill=gray, inner sep=1pt, label=right:{\(|\cdot|_{11}^\alpha\)}] at (3.5,-1) {};
            \node[circle, fill=magenta, inner sep=1pt, label=left:{\(|\cdot|_{2}\)}] at (-1,-1) {};
            \node at (9,-2) {\(\cdots\)};
            \node at (6,-1) {\(\cdots\)};
        \end{tikzpicture}
        \end{center}
...
\subsection{Reduction map and kernel map}
    ...
    \begin{example}\label{eg:Berkovich_spectrum_of_Tate_algebra_1_dim_under_reduction_map}
        Let \(\kkk\) be a complete algebraically closed non-archimedean field.
    \end{example}
    ...
    \begin{example}\label{eg:Tate_algebra_in_1_variable_under_reduction_map}
        Let \(\kkk\) be a complete algebraically closed non-archimedean field and \(A = \kkk\{T\}\).
        We have \(\widetilde{A} \cong \calk_\kkk[T]\).
        For a point \(x_a \in \scrM(A)\) of type I corresponding to \(a \in \kkk\) with \(|a| \leq 1\) (see \cref{cons:four_types_of_points_in_spectrum_of_Tate_algebra_in_one_variable}), 
        the induced homomorphism \(\widetilde{A} \to \calk_{\scrH(x_a)}\) is given by 
    \end{example}
...
\subsection{Spectrum of Tate algebras}
    ...
    \begin{proposition}\label{prop:the_complete_residue_field_of_all_four_types_points_in__spectrum_of_Tate_algebra_in_one_variable}
        ...
        \begin{itemize}
            \item type I point \(x_a\): \(\scrH(x_a)\) is isomorphic to \(\kk\);
            \item type II point \(x_{a,s}\): \(\scrH(x_{a,s}) \cong \calk_\kk((t))\);
            \item type III point \(x_{a,s}\): \(\calk_{\scrH(x_{a,s})} \cong \calk_\kk\) and the value group \(|\scrH(x_{a,s})^\times|\) is generated by \(|\kk^\times|\) and \(s\);
            \item type IV point \(x_{\{E^{(s)}\}}\): \(\scrH(x_{\{E^{(s)}\}})\) is an immediate extension of \(\kk\).
        \end{itemize}
        \Yang{To be checked.}
    \end{proposition}

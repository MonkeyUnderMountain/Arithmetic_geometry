% !TeX root = convergent-and-restricted-power-series.tex
...
\subsection{Absolutely convergent power series}
    ...
    \begin{definition}\label{def:ring_of_absolutely_convergent_power_series}
        ...
        We define the \emph{ring of absolutely convergent power series} over \(R\) with radius \(r\) as
        ...
        \[ R\left<\underline{T/r}\right> \coloneq R\left<T_1/r_1, \ldots, T_n/r_n\right> \coloneq R\left<T_1/r, \ldots, T_{n-1}/r_{n-1}\right>\left<T_n/r_n\right>. \]
    \end{definition}
    ...
    \begin{proposition}\label{prop:ring_of_absolutely_convergent_power_series_is_banach_ring}
        Let \(R\) be a banach ring and \(r > 0\) be a real number.
        Then the ring of absolutely convergent power series \(R\left<T/r\right>\) is a banach ring.
        \Yang{To be checked.}
    \end{proposition}

    In general, the ring of absolutely convergent power series is not even power-multiplicative.
    The following examples describe the uniformization of the ring of absolutely convergent power series over fields.

    \begin{example}\label{eg:uniformation_of_ring_of_absolutely_convergent_power_series_over_C}
        Let \(A = \bbC\left<T/r\right>\).
        For \(f \in A\), set 
        \[ \|f\|_0 \coloneq \max_{x \in E(0,r)} |f(x)|. \]
        I claim that \(\|\cdot\|_0 = \rho_A(\cdot)\).
        \Yang{To be continued.}
    \end{example}

    \begin{example}\label{eg:uniformation_of_ring_of_absolutely_convergent_power_series_over_NA_fields}
        Let \(\kkk\) be an algebraically closed non-archimedean complete field and \(A = \kkk\left<T/r\right>\).
...
\subsection{Tate algebras}
    ...
    \begin{definition}\label{def:Tate_algebra_over_banach_ring}
        ...
        The \emph{Tate algebra} over \(R\) with radius \(r > 0\) in one variable is defined as
        \[ R\{T/r\} \coloneqq \left\{ \sum_{n=0}^{\infty} a_n T^n \in R[[T]] : |a_n| r^n \to 0 \text{ as } n \to +\infty \right\}. \]
        For a tuple of \(n\) indeterminates \(T = (T_1, \ldots, T_n)\) and a tuple of \(n\) positive real numbers \(r = (r_1, \ldots, r_n)\), we define
        \[ R\{\underline{T/r}\} \coloneq R\{T_1/r_1, \ldots, T_n/r_n\} \coloneq R\{T_1/r_1, \ldots, T_{n-1}/r_{n-1}\}\{T_n/r_n\}. \]
        Elements in \(R\{\underline{T/r}\}\) are also called \emph{restricted power series}.
        % \Yang{To be revised.}
    \end{definition}

    \begin{proposition}\label{prop:Tate_algebra_is_a_banach_algebra_over_R}
        Let \(R\) be a non-archimedean banach ring.
        Then the Tate algebra \(R\{ T/r \} \) is a non-archimedean multiplicative banach \(R\)-algebra with respect to the \emph{gauss norm}
        \[
            \left\| \sum_{n \geq 0} a_n T^n \right\| \coloneq \sup_{n\geq 0} |a_n|r^n = \max_{n \geq 0} |a_n|r^n.
        \]
    \end{proposition}
    \begin{proof}
        \Yang{To be added.}
    ...
    The following examples describe the reduction rings of Tate algebras.
    ...
    \begin{proposition}\label{prop:restricted_power_series_invertible_iff_the_constant_item_is_invertible_and_controls_others}
        Let \(R\) be a non-archimedean banach ring.
        An element \(f \in R\{T/r\}\) is invertible if and only if it is distinguished of degree \(0\).
    \end{proposition}
    \begin{proof}
        Denote \(f = \sum_{n=0}^{\infty} a_n T^n\).
        Multiplying by \(a_0^{-1}\), we can reduce to the case \(a_0 = 1\).
        Let \(g = \sum_{n=0}^{\infty} b_n T^n\) be the inverse of \(f\) in \(R[[T]]\).
        Then we have
        \[ f \cdot g = \sum_{k=0}^{\infty} \left( \sum_{n + m = k} a_n b_m \right) T^k = 1 \implies \forall k \geq 1,\ b_k = - \sum_{\substack{n + m = k \\ n \neq 0}} a_n b_m. \]
        Set \(A = \|f-1\| < 1\).
        We show that for every \(m \in \bbN\), there exists \(B_m > 0\) such that for all \(k \geq B_m\), we have \(\|b_k\| r^k \leq A^m\).
        This implies that \(\|b_k\| r^k \to 0\) as \(k \to +\infty\) and hence \(g \in R\{T/r\}\).

        Note that \(b_0 = 1\).
        By induction on \(k\), we have
        \[ \|b_k\| r^k \leq \max_{\substack{n + m = k \\ n \neq 0}} \|a_n\| r^n \cdot \|b_m\| r^m \leq A \max_{m < k} \|b_m\| r^m \leq A. \]
        Suppose that the claim holds for \(m\).
        There exists \(C_{m+1} \in \bbN\) such that for all \(n \geq C_{m+1}\), we have \(\|a_n\| r^n \leq A^{m+1}\).
        Set \(B_{m+1} = B_m + C_{m+1} + 1\).
        For any \(k \geq B_{m+1}\), we have
        \[ \|b_k\| r^k \leq \max_{\substack{n + m = k \\ n \neq 0}} \|a_n\| r^n \cdot \|b_m\| r^m \leq \max \left\{ A^{m+1}, A \cdot A^m \right\} = A^{m+1} \]
        since either \(n \geq C_{m+1}\) or \(m \geq B_m\).
...
\subsection{Weierstrass preparation}
    ...
    \begin{corollary}\label{cor:Tate_alg_in_one_variable_over_field_is_ED}
    ...
    \begin{proof}
        ...
        We need to find \(q, s \in \kk\{T/r\}\) such that
        \[ f = q \cdot g + s, \quad \deg s < \deg g.\]
        \Yang{To be added.}
    \end{proof}

% !TeX root = normed-rings-and-modules.tex
...
\subsection{Semi-normed algebraic structures}
    ...
    \begin{definition}\label{def:normed_group_rings_modules_and_algebras}
        Let \(A\) be an abelian group (or ring, \(R\)-module, \(R\)-algebra) equipped with a semi-norm \(\|\cdot\|\).
        We say that \(\|\cdot\|\) is a \emph{norm} if \(\forall x \in A, \|x\| = 0 \iff x = 0\).
    \end{definition}

    \begin{definition}\label{def:nonarchimedean_seminorm}
        Let \(A\) be an abelian group (or ring, \(R\)-module, \(R\)-algebra) equipped with a semi-norm \(\|\cdot\|\).
        We say that \(\|\cdot\|\) is \emph{non-archimedean} if we have the strong triangle inequality
    ...
    \begin{example}\label{eg:trivial_normed_rings}
        ...
        This norm is non-archimedean and multiplicative.
    \end{example}
    ...
    \begin{example}\label{eg:ring_of_continuous_function_as_normed_rings}
        ...
        The ring \(C(X, \bbR)\) of continuous real-valued functions on \(X\) equipped with the norm \(\|f\| = \sup_{x \in X} |f(x)|\) is a normed \(\bbR\)-algebra.
    ...
    \begin{example}\label{eg:number_fields_as_normed_rings_by_embedding_into_matrix_rings}
        Let \(K\) be a number field and \(\calO_K\) be its ring of integers.
        The action of \(K\) on itself by multiplication induces a embedding \(K \hookrightarrow \End_{\bbQ}(K) \cong M_n(\bbQ)\).
        Consider a norm \(\|\cdot\|\) on the matrix ring \(M_n(\bbQ)\) and restrict it to \(K\).
        Then \((K, \|\cdot\|)\) is a normed ring.
        This norm is even not power-multiplicative in general.
    \end{example}
    ...
    \end{proposition}
    \begin{proof}
        We only need to show that \(\|1\|_{R/I} = 1\).
        \Yang{To be added.}
    \end{proof}
...
\subsection{Spectral radius}
    ...
    \begin{example}\label{eg:spectral_radius_normed_rings_number_fields_by_embedding_into_matrix_rings}
        Consider the normed ring \((K, \|\cdot\|)\) in \cref{eg:number_fields_as_normed_rings_by_embedding_into_matrix_rings}.
        Suppose that \(\|\cdot\|\) on \(M_n(\bbQ)\) is given by the \(2\)-norm induced by the euclidean norm on \(\bbQ^n\).
        Then for a matrix \(A \in M_n(\bbQ)\), its spectral radius \(\rho(A)\) is given by the largest absolute value of eigenvalues of \(A\).
        Hence for each \(a \in K\), its spectral radius \(\rho(a)\) is given by the largest absolute value of embeddings of \(a\) into \(\bbC\), i.e.,
        \[
            \rho(a) = \max_{\sigma: K \hookrightarrow \bbC} |\sigma(a)|.
        \]
    \end{example}
...
\subsection{Reduction rings}
    ...
    \begin{definition}\label{def:reduction_ring_of_nonarchimedean_banach_ring}
        ...
        \]
        The \emph{reduction ring} of \(R\) is defined as the quotient ring
    ...
    \begin{example}\label{eg:reduction_ring_of_trivial_normed_rings}
        ...
        Hence the reduction ring \(\widetilde{R}\) is isomorphic to the reduced ring \(R_{\text{red}} = R/\nil(R)\).
    \end{example}

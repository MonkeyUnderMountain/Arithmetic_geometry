% !TeX root = affinoid-algebras.tex
...
\subsection{Strict case}
    ...
    There are three different categories of finite modules over a \(\kk\)-affinoid algebra \(A\):
    \begin{itemize}
    ...
    \begin{theorem}\label{thm:strictly_affinoid_algebra_equivalent_of_categroy_of_finite_banach_module_and_algebraic_module}
        Let \(A\) be a strictly \(\kk\)-affinoid algebra.
        ...
        Then the three categories \(\Banmod_A\), \(\Banmod_A^b\) and \(\module_A\) are equivalent by natural forgetful functors.
        % \Yang{To be revised.}
    \end{theorem}
    \begin{proof}
        \Yang{To be revised.}
